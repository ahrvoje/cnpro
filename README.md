...
> picture one setting at a time, shows the unit panel exactly as it was set, and
> keeps the settings that *failed* in for the reason they failed. Reproducible on
> any SDXL checkpoint: every reference image in them was generated by the same
> model, so there is nothing to download.
...
|  |  |
|---|---|
| **Space** — paint control strength straight onto the image. Rainbow hue is the weight, red 1 → violet 0; brush size, feathering and an eraser are in the same menu. Two surfaces, two questions. On an **input** there is one slot, **G**: which part of *this input* is read at all — its shape gates what the model (or CLIP) is shown of it, and its painted value *becomes* that input's share of the unit — one weight per input, whatever shape you paint it in. On the **Output mask** tab there are four — **G / C / M / F** — and they are the profiles' spatial half: G for the main profile, C/M/F for the coarse/mid/fine bands, so which slots are live follows the profile selector rather than being a second switch to keep in sync. Anything painted means the control never escapes the paint. | ![weight mask](resources/ui-weight-mask.png) |
...
Worked example: **[the banana car](https://ahrvoje.github.io/cnpro/example_1.html)**
is built on them — why a single weight cannot say "bend the shape but keep the
layout", and why the coarse band's usable window turns out to be about 0.06 wide.
...
Worked example: **[four workers, one person](https://ahrvoje.github.io/cnpro/example_2.html)**
takes this apart one setting at a time — the same four references placed in a
workshop that **canny and depth build between them**, and all three partitions
below compared on one seed, flat average included.
...
Averaging the inputs costs you the curve you drew: five at 1/5 each never pull as
hard as one did. So the wave is not shared out, it is **divided** — each input
takes a share, and the shares add up to a constant at *every* step. The total
pull is exactly the envelope on the plot, split two ways or five.

That constant sum is what "partition of unity" means, and it is all the three
families have in common. They differ in *how* the share is handed over.

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="resources/partition-of-unity-dark.png">
  <img alt="Cosine, Fejér and von Mises partitions at two inputs, five inputs, and five inputs converging" src="resources/partition-of-unity.png">
</picture>

Solid curves are the shares — what the editor plots and what each input runs.
The dashed line is their sum, flat at the envelope in every panel.

**Cosine** — the original soft overlap. At five inputs nobody is ever really in
charge: when input 1 peaks its neighbours are still at 65% of theirs, and the
lobes are only 2/5 tall, which is most of the way back to the flat average.
Fine for two or three references, increasingly nominal past that.

**Fejér** (`F`) — the clean hand-off, from the
[Fejér kernel](https://en.wikipedia.org/wiki/Fej%C3%A9r_kernel). At each input's
moment the others are *exactly* zero, so the wave genuinely passes from one
reference to the next. No setting of its own — the shape follows from the input
count — and at two inputs it *is* the cosine, so switching costs nothing. If you
are unsure which to use, use this one.

**von Mises** (`M`) — the overlap becomes a dial: the `exp(κ·cos)` kernel,
normalized across the *n* phase positions rather than integrated over the circle.

```
w_j(θ) = exp(κ·cos(θ − 2πj/n)) / Σ_k exp(κ·cos(θ − 2πk/n))
```

κ runs the pad's x-axis, from every input equally on at 0 to near-hard switching
at 10. Reach for it when neither the cosine's blur nor Fejér's fixed sharpness is
what the image wants.

**Convergence** — bottom row of the figure. Taking turns suits the early steps
and rarely the late ones, where the references have to agree on a single surface
and one that is switched off cannot help. Switch it on and the waves stop
oscillating: they slide onto the even split, arriving at the step you pick with
the pad's x-axis, the y-axis setting how abruptly. Early: they take turns. Late:
they average. The sum never moves — it changes *who* steers, not *how much*.
...
- **Clipboard** — <kbd>Ctrl</kbd>+<kbd>C</kbd> over a canvas copies exactly what
  it is showing, <kbd>Ctrl</kbd>+<kbd>V</kbd> over another one drops it in, and
  right-click offers the same two as a menu. That is how the same reference gets
  into a second Input tab, into another unit, or out to another application.
