"""IP-Adapter / InstantID injection with per-step, per-band and per-depth weighting.
...
class CrossAttentionPatch:
    ...
    #
    # One instance per patched cross-attention site, so `band_lut` is the LUT
    # of the band THIS site belongs to (resolved at install time from its UNet
    ...
    def __call__(self, n, context_attn2, value_attn2, extra_options):
        ...
        # NEGATIVE weights are meaningful here and must survive: Forge always
        # applies IP-Adapter with weight_type "original", where the injection
        # is the plain scalar multiply `out_ip * weight` below, so w < 0
        # SUBTRACTS this reference's attention contribution - the same
        # repulsive control a negative ControlNet strength gives. Only the
        # (near-)zero case may be skipped, hence a magnitude test; a `w <= 0`
        # skip silently threw away every profile the editor's negative scale
        # range can draw. (Under weight_type "linear" / "channel penalty" the
        ...
        for weight, cond, uncond, ipadapter, mask, weight_type, sigma_start, sigma_end, unfold_batch, weight_profile_lut, balance_lut, band_lut, depth_lut in zip(self.weights, self.conds, self.unconds, self.ipadapters, self.masks, self.weight_type, self.sigma_start, self.sigma_end, self.unfold_batch, self.weight_profile_luts, self.balance_luts, self.band_luts, self.depth_luts):
            ...
            if weight_profile_lut is not None:
                # per-step weight from the control weight profile
                weight = lookup_weight_profile_strength(weight_profile_lut[0], weight_profile_lut[1], sigma)
                if abs(weight) < EPS:
                    continue

            if band_lut is not None:
                # this site's band profile, per step. Main and bands are
                # exclusive (the editor's selector decides): in band mode the
                # extension sends no main profile and a neutral strength 1, so
                # this value IS the per-step strength of this layer. Multiplied
                # rather than assigned so a unit weight set some other way
                # (API, legacy infotext) still scales it.
                weight = weight * lookup_weight_profile_strength(band_lut[0], band_lut[1], sigma)
                if abs(weight) < EPS:
                    continue
            ...
            if balance_lut is not None:
                # per-step prompt/image balance (same semantics as the classic
                # ControlNet balance profile): scale the image-prompt injection
                # per cond (i == 0) / uncond chunk of the batch. The expanded
                # factor tensor is cached on the patch: within one step every
                # patched attention layer needs the identical tensor, so the
                # allocation + host-to-device copy runs once per step instead
                # of once per layer.
                cond_factor, uncond_factor = balance_factors(
                    lookup_weight_profile_strength(balance_lut[0], balance_lut[1], sigma))
                if cond_factor != 1.0 or uncond_factor != 1.0:
                    key = (id(balance_lut), sigma, tuple(cond_or_uncond), batch_prompt,
                           out_ip.dtype, out_ip.device)
                    if getattr(self, '_balance_factor_key', None) != key:
                        factors = [cond_factor if i == 0 else uncond_factor for i in cond_or_uncond]
                        self._balance_factor_key = key
                        self._balance_factor = torch.tensor(
                            factors, dtype=out_ip.dtype, device=out_ip.device
                        ).repeat_interleave(batch_prompt)[:, None, None]
                    out_ip = out_ip * self._balance_factor
...
class IPAdapterApply:
    def apply_ipadapter(self, ipadapter, model, weight, clip_vision=None, image=None, weight_type="original",
        ...
        # attn_mask may be a single tensor (one restriction for every patched
        # site) or a dict band -> tensor: the ControlNet extension's per-band
        # weight masks address the same depths these attention sites sit at, so
        # each site takes the mask of ITS OWN band instead of the union of all
        # painted bands. Restrict-to-painted then holds per band: a band with
        # no mask contributes nothing, which is expressed by not patching its
        # sites at all (see the install loops below).
        band_attn_masks = None
        if isinstance(attn_mask, dict):
            band_attn_masks = {band: m.to(self.device) for band, m in attn_mask.items()
                               if m is not None}
            attn_mask = None
        ...
        # per-step weight from the control weight profile, looked up by sigma
        weight_profile_lut = None
        if weight_profile:
            weight_profile_lut = build_weight_profile_lookup(weight_profile, model.model.predictor.percent_to_sigma)

        # per-step cond/uncond balance profile, same lookup machinery
        balance_lut = None
        if balance_profile:
            balance_lut = build_weight_profile_lookup(balance_profile, model.model.predictor.percent_to_sigma)

        # per-band (coarse/mid/fine) step profiles, one LUT per band. Each
        # patched site picks the LUT of the band its UNet block falls in, so a
        # band profile scales exactly the attention layers of that depth -
        # the same thing it does to a ControlNet's residuals of that depth.
        band_luts = None
        if band_profiles:
            band_luts = {
                band: build_weight_profile_lookup(points, model.model.predictor.percent_to_sigma)
                for band, points in band_profiles.items() if points
            }

        def band_lut_for(block_type, block_id, group_ids):
            if not band_luts:
                return None
        ...
        def site_mask_for(block_type, block_id, group_ids):
            ...
            With per-band masks an ABSENT band means zero control for its
            sites - expressed by skipping the patch entirely, which is both
            the correct weight and cheaper than injecting through a zero mask.
            """
            if band_attn_masks is None:
                return attn_mask, True
            band = band_of_unet_block(block_type, block_id, group_ids)
            mask = band_attn_masks.get(band)
        ...
        patch_kwargs = {
            ...
            "weight_profile_lut": weight_profile_lut,
        ...
        # the stacked transformer blocks of one UNet block all sit at the same
        # resolution, so they share a band.
        def install(block_type, block_id, group_ids, address):
            """Patch one attention site, unless its band is masked away.

            `number` counts EVERY site whether patched or not: it indexes the
            adapter's to_k/to_v layers, so skipping a site must not renumber
            the ones after it.
            """
            mask, patch_it = site_mask_for(block_type, block_id, group_ids)
            if patch_it:
        ...
        if not self.is_sdxl:
            input_ids = [1, 2, 4, 5, 7, 8]  # id of input_blocks that have cross attention
            output_ids = [3, 4, 5, 6, 7, 8, 9, 10, 11]  # id of output_blocks that have cross attention
            for id in input_ids:
                install("input", id, input_ids, ("input", id))
            for id in output_ids:
                install("output", id, output_ids, ("output", id))
            install("middle", 0, [0], ("middle", 0))
        else:
            input_ids = [4, 5, 7, 8]  # id of input_blocks that have cross attention
            output_ids = list(range(6))  # id of output_blocks that have cross attention
            for id in input_ids:
                block_indices = range(2) if id in [4, 5] else range(10)  # transformer_depth
                for index in block_indices:
                    install("input", id, input_ids, ("input", id, index))
            for id in output_ids:
                block_indices = range(2) if id in [3, 4, 5] else range(10)  # transformer_depth
                for index in block_indices:
                    install("output", id, output_ids, ("output", id, index))
            for index in range(10):
                install("middle", 0, [0], ("middle", 0, index))
